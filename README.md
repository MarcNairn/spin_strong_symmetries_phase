...
- [Physical significance & experimental remarks](#physical-significance--experimental-remarks)  
...
- Consequences:
  ...
  - Protected subspaces, non-ergodicity, or long-lived oscillatory modes may appear depending on the Liouvillian spectrum.
...
- \(\{\hat S_\varphi,\hat S_\varphi^\dagger,\hat S_\varphi^z\}\) form an \(\mathfrak{su}(2)\) algebra.
- The Casimir
  \[
...
1. Choose collective scaling with system size \(N\).
2. Solve either the full cavity master equation (small \(N\)) or the reduced atom-only equations.
...
## Physical significance & experimental remarks
- The phase \(\varphi\) is an experimentally accessible control parameter in cavity QED and related platforms.
- It enables steering between bright and dark manifolds, lowering drive thresholds and stabilizing nonstationary phases.
- Applications include protected subspaces, state-dependent dynamics, and engineered dissipation.
